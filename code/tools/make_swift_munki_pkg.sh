#!/bin/bash
#
# Build script for Swift munki tools, builds a distribution package.
...
VERSIONFILE="$MUNKIROOT/code/cli/munki/shared/version.swift"
...
if [ -f "${VERSIONFILE}" ]; then
    ...
    echo "${VERSIONFILE} is missing" 1>&2
    exit 1
fi
...
if [ $LAUNCHDSVNREV -gt $DISTPKGSVNREV ] ; then
    DISTPKGSVNREV=$LAUNCHDSVNREV
fi
...
if [ $PYTHONSVNREV -gt $DISTPKGSVNREV ] ; then
    DISTPKGSVNREV=$PYTHONSVNREV
fi
if [ "$PYTHONPKG" == "YES" ] ; then
    # Get Python version
...
echo
echo "  metapackage version: $DISTPKGVERSION"
echo
...
if [ "$APPSIGNINGCERT" != "" ] ; then
    echo "  Sign code with keychain cert: $APPSIGNINGCERT"
else
    echo "  Sign code: NO"
fi
...
PYTHONREF=""
if [ "$PYTHONPKG" == "YES" ] ; then
    PYTHONOUTLINE="<line choice=\"python\"/>"
    PYTHONCHOICE="<choice id=\"python\" title=\"$PYTHONTITLE\" description=\"$PYTHONDESC\">
...
if [ "$?" -ne 0 ]; then
    echo "Error creating $DISTPKG."
    echo "Attempting to clean up temporary files..."
    sudo rm -rf "$PKGTMP"
    exit 2
fi

echo "Distribution package created at $DISTPKG."
echo
